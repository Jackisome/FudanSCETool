...
public class ProjectService {
    ...
    public float estimateStage1(Project project, Stage1 stage, String model) {
        StackTraceElement[] stackTrace = new Throwable().getStackTrace();
        System.out.println(stackTrace[1].getMethodName());

        float weight = 0.91 + 0.01 * (changeScaleFactorToRate("PREC", project.getPREC()) + changeScaleFactorToRate("FLEX", project.getFLEX()) + changeScaleFactorToRate("RESL", project.getRESL()) + changeScaleFactorToRate("TEAM", project.getTEAM()) + changeScaleFactorToRate("PMAT", project.getPMAT()));
        if (model.equals("model1")) {
            new COCOMOIIModel().estimateStage1(weight, stage);
        }
        else if (model.equals("model2")) {
            new AnalogyModel().estimateStage1(weight, stage);
        }
        else if (model.equals("model3")) {
            new MLPModel().estimateStage1(weight, stage);
        }
        else {
            new COCOMOIIModel().estimateStage1(weight, stage);
        }
    ...
    public float estimateStage2(Project project, Stage2 stage, String model) {
        StackTraceElement[] stackTrace = new Throwable().getStackTrace();
        System.out.println(stackTrace[1].getMethodName());

        float weight = 0.91 + 0.01 * (changeScaleFactorToRate("PREC", project.getPREC()) + changeScaleFactorToRate("FLEX", project.getFLEX()) + changeScaleFactorToRate("RESL", project.getRESL()) + changeScaleFactorToRate("TEAM", project.getTEAM()) + changeScaleFactorToRate("PMAT", project.getPMAT()));
        if (model.equals("model1")) {
            new COCOMOIIModel().estimateStage2(weight, stage);
        }
        else if (model.equals("model2")) {
            new AnalogyModel().estimateStage2(weight, stage);
        }
        else if (model.equals("model3")) {
            new MLPModel().estimateStage2(weight, stage);
        }
        else {
            new COCOMOIIModel().estimateStage2(weight, stage);
        }
    ...
    public float estimateStage3(Project project, Stage3 stage, String model) {
        StackTraceElement[] stackTrace = new Throwable().getStackTrace();
        System.out.println(stackTrace[1].getMethodName());

        float weight = 0.91 + 0.01 * (changeScaleFactorToRate("PREC", project.getPREC()) + changeScaleFactorToRate("FLEX", project.getFLEX()) + changeScaleFactorToRate("RESL", project.getRESL()) + changeScaleFactorToRate("TEAM", project.getTEAM()) + changeScaleFactorToRate("PMAT", project.getPMAT()));
        if (model.equals("model1")) {
            new COCOMOIIModel().estimateStage3(weight, stage);
        }
        else if (model.equals("model2")) {
            new AnalogyModel().estimateStage3(weight, stage);
        }
        else if (model.equals("model3")) {
            new MLPModel().estimateStage3(weight, stage);
        }
        else {
            new COCOMOIIModel().estimateStage3(weight, stage);
        }
    ...
    public void downloadFile(String filename, String id, HttpServletResponse resp, String projectID) {
        StackTraceElement[] stackTrace = new Throwable().getStackTrace();
        System.out.println(stackTrace[1].getMethodName());

        File file = outputProject(projectID);
        if (file.exists()) {
